...
#define HWY_TARGET_INCLUDE "butteraugli.cc"
...
Status Blur(const ImageF& in, float sigma, const ButteraugliParams& params,
  ...
  if (kernel.size() == 5 && &in != out) {
    ...
    JXL_RETURN_IF_ERROR(Separable5(in, Rect(in), weights, out));
    return true;
  }
...
Status SeparateMFAndHF(const ButteraugliParams& params, Image3F* mf, ImageF* hf,
  ...
  MemoryManager* memory_manager = mf[0].memory_manager();
...
Status SeparateHFAndUHF(const ButteraugliParams& params, ImageF* hf,
  ...
  MemoryManager* memory_manager = hf[0].memory_manager();
...
Status SeparateFrequencies(const ButteraugliParams& params, BlurTemp* blur_temp,
                           const Image3F& xyb, PsychoImage& ps) {
  MemoryManager* memory_manager = xyb.memory_manager();
...
template <class Tag>
static Status MaltaDiffMapT(const ImageF& lum0, const ImageF& lum1,
...
Status MaltaDiffMap(const ImageF& lum0, const ImageF& lum1, const double w_0gt1,
  ...
  JXL_RETURN_IF_ERROR(MaltaDiffMapT<MaltaTag>(lum0, lum1, w_0gt1, w_0lt1, norm1,
                                              len, mulli, diffs,
                                              block_diff_ac));
  return true;
}
...
Status MaltaDiffMapLF(const ImageF& lum0, const ImageF& lum1,
  ...
  JXL_RETURN_IF_ERROR(MaltaDiffMapT<MaltaTagLF>(
      lum0, lum1, w_0gt1, w_0lt1, norm1, len, mulli, diffs, block_diff_ac));
  return true;
}
...
Status Mask(const ImageF& mask0, const ImageF& mask1,
  ...
  MemoryManager* memory_manager = mask0.memory_manager();
...
Status MaskPsychoImage(const PsychoImage& pi0, const PsychoImage& pi1,
                       ...
                       ImageF* BUTTERAUGLI_RESTRICT mask,
                       ImageF* BUTTERAUGLI_RESTRICT diff_ac) {
  MemoryManager* memory_manager = pi0.hf[0].memory_manager();
...
Status ButteraugliDiffmapInPlace(Image3F& image0, Image3F& image1,
  ...
  MemoryManager* memory_manager = image0.memory_manager();
  BlurTemp blur_temp;
  {
...
static StatusOr<Image3F> SubSample2x(const Image3F& in) {
  ...
  MemoryManager* memory_manager = in.memory_manager();
...
StatusOr<std::unique_ptr<ButteraugliComparator>> ButteraugliComparator::Make(
  ...
  MemoryManager* memory_manager = rgb0.memory_manager();
  ...
  JXL_RETURN_IF_ERROR(HWY_DYNAMIC_DISPATCH(SeparateFrequencies)(
      params, &result->blur_temp_, xyb0, result->pi0_));

  // Diffmap only blends a single 2x subsample level. Building a full recursive
  // pyramid (old JXL behaviour) kept PsychoImage buffers at every scale down
  // to 8x8 and dominated peak RSS on large inputs. One leaf level is enough.
  if (xsize >= 15 && ysize >= 15) {
    JXL_ASSIGN_OR_RETURN(Image3F subsampledRgb0, SubSample2x(rgb0));
    const size_t sx = subsampledRgb0.xsize();
    const size_t sy = subsampledRgb0.ysize();
    std::unique_ptr<ButteraugliComparator> sub(
        new ButteraugliComparator(sx, sy, params));
    JXL_ASSIGN_OR_RETURN(sub->temp_, Image3F::Create(memory_manager, sx, sy));
    if (sx >= 8 && sy >= 8) {
      JXL_ASSIGN_OR_RETURN(Image3F xyb_sub,
                           Image3F::Create(memory_manager, sx, sy));
      JXL_RETURN_IF_ERROR(HWY_DYNAMIC_DISPATCH(OpsinDynamicsImage)(
          subsampledRgb0, params, sub->Temp(), &sub->blur_temp_, &xyb_sub));
      sub->ReleaseTemp();
      JXL_RETURN_IF_ERROR(HWY_DYNAMIC_DISPATCH(SeparateFrequencies)(
          params, &sub->blur_temp_, xyb_sub, sub->pi0_));
    }
    result->sub_ = std::move(sub);
  }
  return result;
}
...
Status ButteraugliComparator::Diffmap(const Image3F& rgb1,
                                      ImageF& result) const {
  MemoryManager* memory_manager = rgb1.memory_manager();
...
Status ButteraugliComparator::DiffmapOpsinDynamicsImage(const Image3F& xyb1,
                                                        ImageF& result) const {
  MemoryManager* memory_manager = xyb1.memory_manager();
  if (xsize_ < 8 || ysize_ < 8) {
    ZeroFillImage(&result);
    return true;
  }
...
Status ButteraugliComparator::DiffmapPsychoImage(const PsychoImage& pi1,
  ...
  MemoryManager* memory_manager = diffmap.memory_manager();
  if (xsize_ < 8 || ysize_ < 8) {
...
Status ButteraugliDiffmap(const Image3F& rgb0, const Image3F& rgb1,
  ...
  // Make has absorbed rgb0 into pi0_ / sub_; drop pixel storage before Diffmap.
  {
    Image3F drop;
    const_cast<Image3F&>(rgb0).Swap(drop);
  }
...
void ScoreToRgb(double score, double good_threshold, double bad_threshold,
  ...
  score = ba::Clamp1<double>(score * (kTableSize - 1), 0.0, kTableSize - 2);
  int ix = static_cast<int>(score);
  ix = ba::Clamp1(ix, 0, kTableSize - 2);  // Handle NaN
  double mix = score - ix;
  for (int i = 0; i < 3; ++i) {
...
StatusOr<Image3F> CreateHeatMapImage(const ImageF& distmap,
  ...
  MemoryManager* memory_manager = distmap.memory_manager();
  JXL_ASSIGN_OR_RETURN(
      Image3F heatmap,
