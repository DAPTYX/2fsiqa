...
#define HWY_TARGET_INCLUDE "convolve_separable5.cc"
#include <hwy/foreach_target.h>
#include <hwy/highway.h>

#include "convolve-inl.h"
...
class Separable5Impl {
 public:
  ...
  Separable5Impl(const ImageF* in, const Rect& rect,
      ...
      : in(in), rect(rect), weights(weights), out(out) {}
  ...
  Status Run() {
    ...
    if (rect.xsize() >= min_width) {
      ...
      return SlowSeparable5(*in, rect, *weights, out, Rect(*out));
    }
  }
 ...
 private:
  ...
  JXL_INLINE void RunInteriorRows(const size_t ybegin, const size_t yend) {
    ...
    Status status = RunOnPool(ybegin, yend, process_row, "Convolve");
    JXL_DASSERT(status);
    (void)status;
  }
...
Status Separable5(const ImageF& in, const Rect& rect,
                  const WeightsSeparable5& weights, ImageF* out) {
  Separable5Impl impl(&in, rect, &weights, out);
  return impl.Run();
}
...
Status Separable5(const ImageF& in, const Rect& rect,
                  const WeightsSeparable5& weights, ImageF* out) {
  return HWY_DYNAMIC_DISPATCH(Separable5)(in, rect, weights, out);
}
