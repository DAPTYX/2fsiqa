...
// EvalRationalPolynomial (2FS-owned fold of rational_polynomial-inl)
...
struct FastDivision<float, V> {
  V operator()(const V n, const V d) const { return Div(n, d); }
};
...
HWY_INLINE HWY_MAYBE_UNUSED V EvalRationalPolynomial(const D d, const V x,
  ...
  static_assert(kDegP < 8 && kDegQ < 8, "Polynomial degree is too high");
