...
struct PlaneBase {
  ...
  JXL_INLINE MemoryManager* memory_manager() const {
 ...
 protected:
  ...
  Status Allocate(MemoryManager* memory_manager, size_t pre_padding);
...
class Plane : public detail::PlaneBase {
 public:
  ...
  static StatusOr<Plane> Create(MemoryManager* memory_manager,
...
class Image3 {
 public:
  ...
  static StatusOr<Image3> Create(MemoryManager* memory_manager,
                                 const size_t xsize, const size_t ysize) {
...
void ZeroFillImage(Plane<T>* image) {
  ...
  for (size_t y = 0; y < image->ysize(); ++y) {
    std::memset(image->Row(y), 0, image->xsize() * sizeof(T));
  }
}
...
void ZeroFillImage(Image3<T>* image) {
  for (size_t c = 0; c < 3; ++c) {
    ZeroFillImage(&image->Plane(c));
  }
}

static JXL_INLINE int64_t Mirror(int64_t x, const int64_t xsize) {
  while (x < 0 || x >= xsize) {
    if (x < 0) x = -x - 1;
    else x = 2 * xsize - 1 - x;
  }
