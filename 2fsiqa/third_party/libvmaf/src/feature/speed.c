                                 ...
                                 double speed_prescale)
{
    ...
    /* Large images keep DEFAULT_BLOCK_SIZE. For small frames (operating < 5)
     * shrink the block so a non-empty grid still forms; leaves >=80px path identical. */
    {
        const size_t fit = dim->operating_width < dim->operating_height
                               ? dim->operating_width
                               : dim->operating_height;
        if (fit < dim->block_size) {
            if (fit < 1) {
