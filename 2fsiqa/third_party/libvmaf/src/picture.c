...
static int external_release_picture(VmafPicture *pic, void *cookie)
{
    (void) pic;
    (void) cookie;
    return 0;
...
static int picture_fill_geometry(VmafPicture *pic, enum VmafPixelFormat pix_fmt,
...
int vmaf_picture_alloc(VmafPicture *pic, enum VmafPixelFormat pix_fmt,
                       unsigned bpc, unsigned w, unsigned h)
{
    int err = picture_fill_geometry(pic, pix_fmt, bpc, w, h);
    if (err) return err;
...
int vmaf_picture_wrap(VmafPicture *pic, enum VmafPixelFormat pix_fmt,
                      unsigned bpc, unsigned w, unsigned h,
                      void *data[3], const ptrdiff_t stride[3])
{
    if (!data || !stride) return -EINVAL;
    if (!data[0]) return -EINVAL;
    if (stride[0] <= 0) return -EINVAL;

    int err = picture_fill_geometry(pic, pix_fmt, bpc, w, h);
    if (err) return err;

    if (pic->pix_fmt != VMAF_PIX_FMT_YUV400P) {
        if (!data[1] || !data[2]) return -EINVAL;
        if (stride[1] <= 0 || stride[2] <= 0) return -EINVAL;
    }
    ...
    err |= vmaf_picture_priv_init(pic);
    err |= vmaf_picture_set_release_callback(pic, NULL, external_release_picture);
    if (err) {
        free(pic->priv);
        memset(pic, 0, sizeof(*pic));
        return -ENOMEM;
